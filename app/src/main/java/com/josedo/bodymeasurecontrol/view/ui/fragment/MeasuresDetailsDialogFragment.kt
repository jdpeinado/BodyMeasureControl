...
class MeasuresDetailsDialogFragment : DialogFragment() {
    ...
    private fun setBackgroundColor(difference: Double, view: View) {
        when {
            (difference == 0.0) -> view.background =
                context!!.getDrawable(R.drawable.shape_measurement_blue)
            (difference > 0.0) -> view.background =
                context!!.getDrawable(R.drawable.shape_measurement_red)
            (difference < 0.0) -> view.background =
                context!!.getDrawable(R.drawable.shape_measurement_green)
        }
    }
